...
import { PlatzLandNFT, LandMarketplace } from "../typechain-types";
...
describe("LandMarketplace", function () {
    async function deployContractsFixture() {
        ...
        const landNFT = await PlatzLandNFTFactory.connect(deployer).deploy(deployer.address) as PlatzLandNFT;
        ...
        const marketplace = await LandMarketplaceFactory.connect(deployer).deploy(
            deployer.address, 
            landNFTAddress,
    ...
    describe("Deployment", function () {
        ...
        it("Should set the correct minimum bid increment percentage", async function () {
            const { marketplace, minBidIncrementPercentage } = await loadFixture(deployContractsFixture);
            expect(await marketplace.minBidIncrementPercentage()).to.equal(minBidIncrementPercentage);
        });
    });
    ...
    describe("Fixed-Price Listing: listNFT", function () {
        ...
        it("Should allow NFT owner to list their token", async function () {
            const { marketplace, landNFT, seller } = await loadFixture(deployContractsFixture);
            await landNFT.connect(seller).approve(await marketplace.getAddress(), tokenIdToList);

            await expect(marketplace.connect(seller).listNFT(tokenIdToList, listingPrice))
        ...
        it("Should NOT allow listing if price is zero", async function () {
            const { marketplace, landNFT, seller } = await loadFixture(deployContractsFixture);
            await landNFT.connect(seller).approve(await marketplace.getAddress(), tokenIdToList);
            await expect(marketplace.connect(seller).listNFT(tokenIdToList, 0))
                .to.be.revertedWith("Marketplace: Price must be > 0");
        });

        it("Should NOT allow non-owner to list an NFT", async function () {
            const { marketplace, landNFT, buyer, seller } = await loadFixture(deployContractsFixture);
            // Ensure token 1 is owned by seller, not buyer
            expect(await landNFT.ownerOf(tokenIdToList)).to.equal(seller.address);
            await expect(marketplace.connect(buyer).listNFT(tokenIdToList, listingPrice))
        ...
        it("Should NOT allow listing if token is already listed", async function () {
            ...
            await expect(marketplace.connect(seller).listNFT(tokenIdToList, listingPrice)) // Try listing again
        ...
        it("Should NOT allow listing if marketplace is not approved for the token", async function () {
            const { marketplace, seller } = await loadFixture(deployContractsFixture);
            await expect(marketplace.connect(seller).listNFT(tokenIdToList, listingPrice))
    ...
    describe("Fixed-Price Sale: buyNFT", function () {
        ...
        let setup: any; // To store fixture data for this describe block

        beforeEach(async function () {
            setup = await loadFixture(deployContractsFixture);
        ...
        it("Should allow a user to buy a listed NFT", async function () {
            ...
            await expect(marketplace.connect(buyer).buyNFT(tokenIdToBuy, { value: listingPrice }))
                .to.emit(marketplace, "NFTSold")
                .withArgs(tokenIdToBuy, seller.address, buyer.address, listingPrice);

            expect(await landNFT.ownerOf(tokenIdToBuy)).to.equal(buyer.address);
        ...
        it("Should NOT allow buying with incorrect ETH amount", async function () {
            ...
            await expect(marketplace.connect(buyer).buyNFT(tokenIdToBuy, { value: wrongPrice }))
        ...
        it("Should NOT allow seller to buy their own NFT", async function () {
            ...
            await expect(marketplace.connect(seller).buyNFT(tokenIdToBuy, { value: listingPrice }))
        ...
        it("buyNFT should void an active bid and refund bidder", async function() {
            ...
            const { marketplace, landNFT, seller, buyer, bidder1 } = await loadFixture(deployContractsFixture);
            ...
            expect(highestBid.active).to.be.false;

            const bidder1FinalBalance = await ethers.provider.getBalance(bidder1.address);
            expect(bidder1FinalBalance).to.equal(bidder1InitialBalance + bidAmount);
        });
    });
    ...
    describe("Fixed-Price Listing: cancelListing", function () {
        ...
        it("Should allow lister to cancel their listing", async function () {
            const { marketplace, seller } = setup;
            await expect(marketplace.connect(seller).cancelListing(tokenIdToCancel))
        ...
        it("Should NOT allow non-lister to cancel a listing", async function () {
            const { marketplace, buyer } = setup;
            await expect(marketplace.connect(buyer).cancelListing(tokenIdToCancel))
    ...
    describe("Fixed-Price Listing: updateListingPrice", function () {
        ...
        it("Should allow lister to update the price of their listing", async function () {
            const { marketplace, seller } = setup;
            await expect(marketplace.connect(seller).updateListingPrice(tokenIdToUpdate, newPrice))
        ...
        it("Should NOT allow non-lister to update price", async function () {
            const { marketplace, buyer } = setup;
            await expect(marketplace.connect(buyer).updateListingPrice(tokenIdToUpdate, newPrice))
                .to.be.revertedWith("Marketplace: Not lister");
        });
        ...
        it("Should NOT allow updating price to zero", async function () {
            const { marketplace, seller } = setup;
            await expect(marketplace.connect(seller).updateListingPrice(tokenIdToUpdate, 0))
                .to.be.revertedWith("Marketplace: Price must be > 0");
        });
    });
    ...
    describe("Bidding: makeBid", function () {
        ...
        it("Should allow a user to make a bid on an NFT", async function () {
            const { marketplace, bidder1 } = await loadFixture(deployContractsFixture);
            await expect(marketplace.connect(bidder1).makeBid(tokenIdToBidOn, { value: initialBidAmount }))
            ...
            expect(bid.amount).to.equal(initialBidAmount);
            expect(bid.active).to.be.true;
        });

        it("Should NOT allow owner to bid on their own NFT", async function () {
            const { marketplace, seller } = await loadFixture(deployContractsFixture);
            await expect(marketplace.connect(seller).makeBid(tokenIdToBidOn, { value: initialBidAmount }))
                .to.be.revertedWith("Marketplace: Owner cannot bid");
        });

        it("Should NOT allow bidding on a non-existent token", async function () {
            const { marketplace, bidder1, landNFT } = await loadFixture(deployContractsFixture);
            await expect(marketplace.connect(bidder1).makeBid(nonExistentTokenId, { value: initialBidAmount }))
        ...
        it("Should require new bid to be higher than current highest bid by minBidIncrementPercentage", async function () {
            const { marketplace, bidder1, bidder2, minBidIncrementPercentage } = await loadFixture(deployContractsFixture);
            ...
            await expect(marketplace.connect(bidder2).makeBid(tokenIdToBidOn, { value: lowBidAmount }))
                .to.be.revertedWith("Marketplace: Bid too low");
            
            await expect(marketplace.connect(bidder2).makeBid(tokenIdToBidOn, { value: validNextBidAmount }))
        ...
        it("Should refund previous highest bidder when a new higher bid is made", async function () {
            ...
            const gasPaid1: bigint = receipt1!.gasUsed * receipt1!.gasPrice;
            const bidder1BalanceAfterBid1 = await ethers.provider.getBalance(bidder1.address);
            expect(bidder1BalanceAfterBid1).to.equal(bidder1InitialEthBalance - initialBidAmount - gasPaid1);
            ...
            expect(bidder1FinalEthBalance).to.equal(bidder1InitialEthBalance - gasPaid1);
        });
    });
    ...
    describe("Bidding: acceptBid", function () {
        ...
        it("Should allow NFT owner to accept an active bid", async function () {
            ...
            const sellerInitialBalance: bigint = await ethers.provider.getBalance(seller.address);
            ...
            const gasPaidBySeller: bigint = gasUsed * gasPrice;

            await expect(tx)
                .to.emit(marketplace, "BidAccepted")
                .withArgs(tokenIdToAcceptBidFor, seller.address, bidder1.address, bidAmount);
            
            expect(await landNFT.ownerOf(tokenIdToAcceptBidFor)).to.equal(bidder1.address);
            const bid = await marketplace.getHighestBid(tokenIdToAcceptBidFor);
            expect(bid.active).to.be.false;
            const sellerFinalBalance: bigint = await ethers.provider.getBalance(seller.address);
            // Broke down the arithmetic for TSC
            const sumSellerGain: bigint = sellerInitialBalance + bidAmount;
            const expectedSellerBalance: bigint = sumSellerGain - gasPaidBySeller;
            expect(sellerFinalBalance).to.equal(expectedSellerBalance); 
        });
        ...
        it("Should NOT allow non-owner to accept a bid", async function () {
            ...
            await expect(marketplace.connect(otherUser).acceptBid(tokenIdToAcceptBidFor))
                .to.be.revertedWith("Marketplace: Not owner");
        });
        ...
        it("Should NOT allow accepting a bid if no active bid exists", async function () {
            ...
            await landNFT.connect(seller).approve(await marketplace.getAddress(), tokenIdToAcceptBidFor);
            await expect(marketplace.connect(seller).acceptBid(tokenIdToAcceptBidFor))
    ...
    describe("Bidding: withdrawBid", function () {
        ...
        it("Should allow the highest bidder to withdraw their active bid", async function () {
            const { marketplace, bidder1 } = setup;
            const bidder1InitialBalance: bigint = await ethers.provider.getBalance(bidder1.address);
            const tx = await marketplace.connect(bidder1).withdrawBid(tokenIdToWithdrawFrom);
            const receipt = await tx.wait();
            const gasUsed: bigint = receipt!.gasUsed;
            const gasPrice: bigint = receipt!.gasPrice;
            const gasPaid: bigint = gasUsed * gasPrice;
            ...
            expect(bid.active).to.be.false;
            const bidder1FinalBalance: bigint = await ethers.provider.getBalance(bidder1.address);
            // Broke down the arithmetic for TSC
            const sumBidderRefund: bigint = bidder1InitialBalance + bidAmount;
            const expectedBidderBalance: bigint = sumBidderRefund - gasPaid;
            expect(bidder1FinalBalance).to.equal(expectedBidderBalance);
        });

        it("Should NOT allow non-highest bidder to withdraw a bid", async function () {
            const { marketplace, bidder2 } = setup; 
            await expect(marketplace.connect(bidder2).withdrawBid(tokenIdToWithdrawFrom))
        ...
        it("Should NOT allow withdrawing if bid is not active", async function () {
            ...
            await expect(marketplace.connect(bidder1).withdrawBid(tokenIdToWithdrawFrom))
                .to.be.revertedWith("Marketplace: Not highest bidder or bid inactive");
        });
    });
    ...
    describe("Admin Functions", function() {
        it("Owner should be able to set NFT contract address", async function() {
            const { marketplace, deployer, landNFTAddress } = await loadFixture(deployContractsFixture);
            const newNftAddress = ethers.Wallet.createRandom().address; // Create a random new address
            await expect(marketplace.connect(deployer).setNftContractAddress(newNftAddress))
        ...
        it("Non-owner should NOT be able to set NFT contract address", async function() {
            const { marketplace, otherUser } = await loadFixture(deployContractsFixture);
            const newNftAddress = ethers.Wallet.createRandom().address;
            await expect(marketplace.connect(otherUser).setNftContractAddress(newNftAddress))
        ...
        it("Owner should be able to set minimum bid increment percentage", async function() {
            ...
            expect(await marketplace.minBidIncrementPercentage()).to.equal(newPercentage);
        });

        it("Non-owner should NOT be able to set minimum bid increment percentage", async function() {
            const { marketplace, otherUser } = await loadFixture(deployContractsFixture);
            const newPercentage = 10;
            await expect(marketplace.connect(otherUser).setMinBidIncrementPercentage(newPercentage))
                .to.be.revertedWithCustomError(marketplace, "OwnableUnauthorizedAccount").withArgs(otherUser.address);
        });
    });
