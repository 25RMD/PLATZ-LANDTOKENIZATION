...
/**
 * Script to fetch the 2 earliest listings from the database
 * Usage: node scripts/get-earliest-listings.js
 */
...
async function getEarliestListings() {
  try {
    console.log('🔍 Fetching the 2 earliest listings from the database...\n');

    const earliestListings = await prisma.land_listings.findMany({
      orderBy: {
        created_at: 'asc'
      },
      take: 2,
...
module.exports = { getEarliestListings };
