  ...
  try {
    ...
    const collections = await prisma.landListing.findMany({
      ...
      select: {
        ...
        mainTokenId: true,
        metadataUri: true,
        evmOwnerAddress: true,
        isListedForSale: true,
        listingPriceEth: true,
  ...
  try {
    const collection = await prisma.landListing.findUnique({
      ...
      select: {
        ...
        evmOwnerAddress: true,
        isListedForSale: true,
        listingPriceEth: true,   // Prisma Decimal type
        mintTransactionHash: true,
        mintTimestamp: true,
        createdAt: true,
        user: {
    ...
    const serializableCollection = {
      ...collection,
      listingPrice: collection.listingPrice ? (collection.listingPrice as any as import('@prisma/client').Prisma.Decimal).toNumber() : null,
      listingPriceEth: collection.listingPriceEth ? (collection.listingPriceEth as any as import('@prisma/client').Prisma.Decimal).toNumber() : null,
      collectionId: collection.collectionId?.toString(),
      mainTokenId: collection.mainTokenId?.toString(),
