...
describe("PlatzLandNFT", function () {
    ...
    describe("Deployment", function () {
        it("Should set the right owner", async function () {
            (0, chai_1.expect)(await platzLandNFT.owner()).to.equal(owner.address);
        });
        it("Should have correct name and symbol", async function () {
            (0, chai_1.expect)(await platzLandNFT.name()).to.equal("Platz Land Token");
            (0, chai_1.expect)(await platzLandNFT.symbol()).to.equal("PLTZ");
        });
    });
    describe("Minting", function () {
        it("Should mint a new token", async function () {
            ...
            await mintTx.wait();
            (0, chai_1.expect)(await platzLandNFT.ownerOf(0)).to.equal(addr1.address);
            (0, chai_1.expect)(await platzLandNFT.tokenURI(0)).to.equal(tokenURI);
        });
        it("Should emit PropertyMinted event on minting", async function () {
            await (0, chai_1.expect)(platzLandNFT.mintLand(addr1.address, propertyReference, tokenURI))
        ...
        it("Should store property details correctly", async function () {
            ...
            (0, chai_1.expect)(propertyDetails.propertyReference).to.equal(propertyReference);
            (0, chai_1.expect)(propertyDetails.creator).to.equal(owner.address);
            ...
            (0, chai_1.expect)(Number(propertyDetails.mintTimestamp)).to.be.greaterThan(jan2023Timestamp);
        });
        it("Should mint multiple tokens with incremental IDs", async function () {
            ...
            (0, chai_1.expect)(await platzLandNFT.ownerOf(0)).to.equal(addr1.address);
            (0, chai_1.expect)(await platzLandNFT.ownerOf(1)).to.equal(addr2.address);
            (0, chai_1.expect)(await platzLandNFT.tokenURI(0)).to.equal("uri1");
            (0, chai_1.expect)(await platzLandNFT.tokenURI(1)).to.equal("uri2");
        });
    });
    describe("Token existence", function () {
        it("Should correctly report token existence", async function () {
            await platzLandNFT.mintLand(addr1.address, propertyReference, tokenURI);
            (0, chai_1.expect)(await platzLandNFT.exists(0)).to.be.true;
            (0, chai_1.expect)(await platzLandNFT.exists(1)).to.be.false;
        });
    });
    describe("Burning", function () {
        ...
        it("Should allow token owner to burn their token", async function () {
            ...
            await (0, chai_1.expect)(platzLandNFT.ownerOf(0)).to.be.revertedWithCustomError(platzLandNFT, "ERC721NonexistentToken");
            (0, chai_1.expect)(await platzLandNFT.exists(0)).to.be.false;
        });
        ...
        it("Should not allow unauthorized addresses to burn tokens", async function () {
            await (0, chai_1.expect)(platzLandNFT.connect(addr2).burn(0)).to.be.revertedWith("PlatzLandNFT: caller is not owner nor approved");
        });
    });
    describe("Property details", function () {
        ...
        it("Should return property details for existing token", async function () {
            ...
            (0, chai_1.expect)(details.propertyReference).to.equal(propertyReference);
        });
        it("Should revert when querying non-existent token", async function () {
            await (0, chai_1.expect)(platzLandNFT.getPropertyDetails(999)).to.be.revertedWithCustomError(platzLandNFT, "ERC721NonexistentToken");
        });
    });
});
